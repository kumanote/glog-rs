...
impl<D: Decorator, C: KVCategorizer> GlogFormat<D, C> {
    pub fn new(decorator: D, categorizer: C) -> Self {
        Self {
            decorator,
            categorizer,
        }
...
impl<D: Decorator, C: KVCategorizer> Drain for GlogFormat<D, C> {
    ...
    fn log(&self, record: &Record<'_>, values: &OwnedKVList) -> std::io::Result<Self::Ok> {
...
fn finish_logline(decorator: &mut dyn RecordDecorator) -> std::io::Result<()> {
    decorator.start_whitespace()?;
    writeln!(decorator)?;
    decorator.flush()
}
